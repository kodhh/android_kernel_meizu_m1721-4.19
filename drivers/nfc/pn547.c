...
static int pn547_clock_select(struct pn547_dev *pn547_dev)
{
	...
	if (pn547_dev->clk_run == false) {
		...
		if (r) {
			pr_err("pn547 clk_prepare_enable fail\n");
			goto err_invalid_clk;
		}
		pr_err("pn547 clk_prepare_enable ok\n");
...
{	
	...
	pn547_dev->clk_run = false;
	temp = pn547_clock_select(pn547_dev);
	printk("lj: temp:%d  ====\n", temp);
	temp = gpio_is_valid(pn547_dev->clkreq_gpio);
	printk("lj: gpio_is_valid pn547_dev->clkreq_gpio:%ud : temp:%d \n", pn547_dev->clkreq_gpio , temp);
